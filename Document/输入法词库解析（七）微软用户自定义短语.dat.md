...
source: "https://nopdan.com/2022/09/07-msudp-dat/"
author:
  - "[[nopdan]]"
published: 2022-09-17
created: 2026-05-27
description: "分享编程和输入法"
tags:
  - "clippings"
topics:
---
>[!info] 总结
> 该文档详细介绍了微软拼音和五笔输入法通用的用户自定义短语 `.dat` 文件格式。核心结构包括：文件头（8字节标识 `machxudp`）、版本号、偏移表（记录各词条位置）和词条本体。每个词条包含固定标记（`10 00 10 00`）、编码长度、候选顺序、时间戳，以及UTF-16LE编码的编码和词语。文档最后提供了Go语言实现的解析（`Parse`）和生成（`Gen`）函数代码，用于处理该格式文件。
...
接下来每 4 字节一组，分别表示 *偏移表开始* 、 *词条开始* 、 *文件总长* 、 *词条数* 、 *导出的时间戳* 。
