...
def upload_files_to_s3(config_params):
    ...
    try:
                        ...
                        try:
                            ...
                            if result.returncode == 0:
                                converted_file = os.path.join(
                                    project_root, "out", "ChsPinyinEUDPv1.txt"
                                )
                                if os.path.isfile(converted_file):
                                    files_to_upload.append(
                                        ("ChsPinyinEUDPv1.txt", converted_file)
                                    )
                                else:
                                    files_to_upload.append(
                                        ("ChsPinyinEUDPv1.lex", lex_file_path)
                                    )
                            else:
                                files_to_upload.append(
                                    ("ChsPinyinEUDPv1.lex", lex_file_path)
                    ...
                    if os.path.isfile(file_path):
                        ...
                        if file_name != "upload_to_s3.py" and not file_name.startswith(
                            ...
                            if not any(
                                ...
                                for existing_name, _ in files_to_upload
                            ):
        ...
        if success_count > 0:
            ...
            # 如果上传了 lex 文件，显示安装命令
            lex_uploaded = any(
                file_name in ["ChsPinyinEUDPv1.lex", "ChsPinyinEUDPv1.txt"]
                for file_name, _ in files_to_upload
            )
            if lex_uploaded:
