...
def cmd_debug(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if Path(lex_path).exists():
            ...
            print(
                f"最后修改：{datetime.fromtimestamp(file_stat.st_mtime).strftime('%Y-%m-%d %H:%M:%S')}"
            )
...
def cmd_convert(args: argparse.Namespace) -> int:
    ...
    # === 路径规范化 ===
    input_path = os.path.normpath(args.input.strip()) if args.input else None
    output_dir = os.path.normpath(args.output.strip()) if args.output else "out"
    ...
    if getattr(sys, "frozen", False):
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                "phrase_converter", str(convert_script)
            )
            ...
            # 执行转换
            if args.format and input_path:
                success = convert_module.convert_phrases(
                    args.format, input_path, output_dir
                )
...
def cmd_delete(args: argparse.Namespace) -> int:
    ...
    if getattr(sys, "frozen", False):
        ...
        try:
            ...
            # 动态导入模块
            spec = importlib.util.spec_from_file_location(
                "delete_microsoft_phrases", str(delete_script)
            )
...
def cmd_upload(args: argparse.Namespace) -> int:
    ...
    if getattr(sys, "frozen", False):
        ...
        try:
            ...
            # 动态导入模块
            spec = importlib.util.spec_from_file_location(
                "upload_to_s3", str(upload_script)
            )
            ...
            # 调用上传函数
            print(
                f"📋 存储桶: {upload_module.CONFIG['S3_BUCKET_NAME']} 目录: {upload_module.CONFIG['S3_DIRECTORY']}"
            )
...
def cmd_edit(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if result["moved"]:
            print(
                f"✓ 已将文本 '{text}' 从索引 {result['old_index']} 移动到索引 {index}"
            )
        elif result["updated"]:
...
""",
    ...
    convert_parser = subparsers.add_parser(
        "convert", help="短语类型转换（百度/搜狗/微软/Rime/多多/CSV互转）"
    )
    ...
    delete_parser.add_argument(
        "--force", "-f", action="store_true", help="强制删除，不提示确认"
    )
    delete_parser.add_argument(
        "--dry-run", "-n", action="store_true", help="只显示将要删除的文件，不实际删除"
    )
